#include "libufs_internel.h"

static void _trans_blist(_blist_item_t* item) {
    int i;
    item->num = ul_trans_i64_le(item->num);
    for(i = 0; i < UFS_BLIST_ENTRY_NUM_MAX; ++i)
        item->stack[i] = ul_trans_u64_le(item->stack[i]);
}
static _blist_item_t* _todisk_alloc(const _blist_item_t* item) {
    int i;
    _blist_item_t* ret = ul_reinterpret_cast(_blist_item_t*, ufs_malloc(UFS_BLOCK_SIZE));
    if(ul_unlikely(ret == NULL)) return ret;
    ret->num = ul_trans_i64_le(ul_static_cast(int64_t, ul_static_cast(uint64_t, item->num) | (item->flag ? 0x8000000000000000u : 0u)));
    for(i = 0; i < UFS_BLIST_ENTRY_NUM_MAX; ++i)
        ret->stack[i] = ul_trans_u64_le(item->stack[i]);
    return ret;
}

static int _read_blist(_blist_item_t* item, ufs_bcache_t* bcache, uint64_t bnum) {
    int ec;
    ec = ufs_bcache_read(bcache, item, bnum);
    if(ul_unlikely(ec)) return ec;

    _trans_blist(item);
    if(item->num > UFS_BLIST_ENTRY_NUM_MAX) return UFS_ERROR_BROKEN_DISK;

    item->flag = (ul_static_cast(uint64_t, item->num) & 0x8000000000000000u) != 0;
    item->num = item->num & 0x7FFFFFFFFFFFFFFF;
    item->bnum = bnum;
    return 0;
}
static int _write_blist(const _blist_item_t* item, ufs_bcache_t* bcache) {
    _blist_item_t* ret;
    ret = _todisk_alloc(item);
    if(ul_unlikely(ret == NULL)) return ENOMEM;
    return ufs_bcache_add(bcache, ret, item->bnum, UFS_BCACHE_ADD_MOVE);
}

static int _expand_blist_pop(ufs_blist_t* blist) {
    int i = blist->n, ec = 0;
    if(ul_unlikely(i == 0)) return ENOSPC;
    for(; ul_likely(i < UFS_BLIST_STACK_MAX) && blist->item[i - 1].flag == 0; ++i) {
        if(ul_unlikely(blist->item[i - 1].num == 0)) return UFS_ERROR_BROKEN_DISK;
        ec = _read_blist(blist->item + i, blist->bcache, blist->item[i - 1].stack[blist->item[i - 1].num - 1]);
        if(ul_unlikely(ec)) return ec;
    }
    blist->n = i;
    return 0;
}
static int _expand_blist_push_empty(ufs_blist_t* blist, int i) {
    int ec = 0;
    for(; ul_likely(i < UFS_BLIST_STACK_MAX) && blist->item[i - 1].flag == 0; ++i) {
        if(ul_unlikely(blist->item[i - 1].num == UFS_BLIST_ENTRY_NUM_MAX)) return UFS_ERROR_BROKEN_DISK;
        blist->item[i - 1].num = 1;
        ec = _read_blist(blist->item + i, blist->bcache, blist->item[i - 1].stack[0]);
        if(ul_unlikely(ec)) return ec;
    }
    blist->n = i;
    return 0;
}
static int _expand_blist_push(ufs_blist_t* blist) {
    int i = blist->n, ec = 0;
    if(ul_unlikely(i == 0)) return _expand_blist_push_empty(blist, 1);
    for(; ul_likely(i < UFS_BLIST_STACK_MAX) && blist->item[i - 1].flag == 0; ++i) {
        if(blist->item[i - 1].num == 0) return _expand_blist_push_empty(blist, i);
        ec = _read_blist(blist->item + i, blist->bcache, blist->item[i - 1].stack[blist->item[i - 1].num - 1]);
        if(ul_unlikely(ec)) return ec;
    }
    blist->n = i;
    return 0;
}

UFS_HIDDEN int ufs_blist_init(ufs_blist_t* blist, ufs_bcache_t* bcache, uint64_t start) {
    int ec;
    blist->bcache = bcache;
    ec = _read_blist(blist->item, blist->bcache, start);
    if(ul_unlikely(ec)) return ec;
    if(blist->item[0].num == 0) blist->n = 0;
    else {
        blist->n = 1;
        ec = _expand_blist_pop(blist);
        if(ul_unlikely(ec)) return ec;
    }
    ulatomic_spinlock_unlock(&blist->lock);
    return 0;
}
UFS_HIDDEN void ufs_blist_deinit(ufs_blist_t* blist) {
    (void)blist;
}
UFS_HIDDEN int ufs_blist_sync(ufs_blist_t* blist) {
    int i, ec = 0;
    ulatomic_spinlock_lock(&blist->lock);

    for(i = 0; i < blist->n; ++i) {
        ec = _write_blist(blist->item + i, blist->bcache);
        if(ul_unlikely(ec)) goto do_return;
    }

do_return:
    ulatomic_spinlock_unlock(&blist->lock);
    return ec;
}
UFS_HIDDEN int ufs_blist_pop(ufs_blist_t* blist, uint64_t* pbnum) {
    int i, ec = 0;
    ulatomic_spinlock_lock(&blist->lock);

    for(i = blist->n; i > 0; --i) {
        if(blist->item[i - 1].num > 0) break;
        ec = _write_blist(blist->item + i - 1, blist->bcache);
        if(i > 1) --blist->item[i - 2].num;
    }
    blist->n = i;
    ec = _expand_blist_pop(blist);
    if(ul_unlikely(ec)) goto do_return;
    *pbnum = blist->item[blist->n - 1].stack[--blist->item[blist->n - 1].num];

do_return:
    ulatomic_spinlock_unlock(&blist->lock);
    return ec;
}
UFS_HIDDEN int ufs_blist_push(ufs_blist_t* blist, uint64_t bnum) {
    int i, ec = 0;
    ulatomic_spinlock_lock(&blist->lock);

    for(i = blist->n; i > 0; --i) {
        if(blist->item[i - 1].num < UFS_BLIST_ENTRY_NUM_MAX) break;
        ec = _write_blist(blist->item + i - 1, blist->bcache);
        if(ul_unlikely(ec)) goto do_return;
    }
    if(i != blist->n && i > 0) ++blist->item[i - 1].num;
    blist->n = i;
    ec = _expand_blist_push(blist);
    if(ul_unlikely(ec)) goto do_return;
    blist->item[blist->n - 1].stack[blist->item[blist->n - 1].num++] = bnum;

do_return:
    ulatomic_spinlock_unlock(&blist->lock);
    return ec;
}
